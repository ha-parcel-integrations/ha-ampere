"""Calendar platform for the Ampère parcel tracker integration."""
...
async def async_setup_entry(
    ...
    """Set up the Ampère deliveries calendar from a config entry."""
    async_add_entities([AmpReDeliveriesCalendar(entry.runtime_data.coordinator, entry)])


class AmpReDeliveriesCalendar(CoordinatorEntity[AmpReCoordinator], CalendarEntity):
    """A read-only calendar of expected Ampère deliveries.
    ...
    def __init__(self, coordinator: AmpReCoordinator, entry: ConfigEntry) -> None:
    ...
    def _events(self) -> list[CalendarEvent]:
        ...
        for parcel in self.coordinator.data or []:
            ...
            summary = sender or (f"Parcel {barcode}" if barcode else "Ampère parcel")
            description_parts = [
