"""Sensor platform for the Ampère parcel tracker integration."""
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: AmpReConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up Ampère sensor entities from a config entry."""
    ...
    entities: list[SensorEntity] = [
        AmpReIncomingParcelsSensor(
            coordinator, entry, async_add_entities, current_barcodes
        ),
    ]
    for parcel in coordinator.data or []:
        entities.append(
            AmpReParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(AmpReNextDeliverySensor(coordinator, entry))
    entities.append(AmpReDeliveredParcelsSensor(coordinator, entry))
    entities.append(AmpReLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class AmpReIncomingParcelsSensor(
    CoordinatorEntity[AmpReCoordinator], SensorEntity
):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                AmpReParcelSensor(self.coordinator, self._entry, barcode)
...
class AmpReParcelSensor(CoordinatorEntity[AmpReCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked Ampère parcel."""
    ...
    def __init__(
        self, coordinator: AmpReCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class AmpReNextDeliverySensor(
    ...
    def __init__(
        self, coordinator: AmpReCoordinator, entry: ConfigEntry
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class AmpReDeliveredParcelsSensor(
    CoordinatorEntity[AmpReCoordinator], SensorEntity
):
    """Recently delivered tracked Ampère parcels."""
...
class AmpReLastUpdateSensor(
    CoordinatorEntity[AmpReCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when Ampère was last polled successfully."""
