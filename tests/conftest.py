"""pytest configuration for the Ampère test suite."""
import sys
...
def auto_enable_custom_integrations(enable_custom_integrations):
    """Make ``custom_components.ampere`` loadable from config-flow / setup tests."""
    yield


@pytest.fixture(autouse=True)
def reset_one_shot_warnings():
    """Clear the "already warned this session" state between tests.

    Both are module-level by design -- a user must not be told about the
    same unmapped status or history-length mismatch on every poll -- but
    that also makes them leak across tests, so whether a warning fires
    would otherwise depend on test order.
    """
    from custom_components.ampere import parcels

    parcels._unmapped_statuses_logged.clear()
    parcels._history_mismatch_warned = False
    yield
