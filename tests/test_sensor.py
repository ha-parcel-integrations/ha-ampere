"""Tests for Ampère sensor property logic."""
from datetime import datetime, timezone
from unittest.mock import MagicMock

from custom_components.ampere.const import ParcelStatus
from custom_components.ampere.sensor import (
    AmpReDeliveredParcelsSensor,
    AmpReIncomingParcelsSensor,
    AmpReLastUpdateSensor,
    AmpReNextDeliverySensor,
    AmpReParcelSensor,
)
...
def test_incoming_counts_and_lists():
    ...
    sensor = AmpReIncomingParcelsSensor(coordinator, _entry(), lambda _: None, set())
...
def test_parcel_sensor_status_and_attributes():
    ...
    sensor = AmpReParcelSensor(_coordinator([parcel]), _entry(), "A")
...
def test_parcel_sensor_missing_barcode():
    sensor = AmpReParcelSensor(_coordinator([_parcel("A")]), _entry(), "OTHER")
...
def test_next_delivery_picks_earliest():
    ...
    sensor = AmpReNextDeliverySensor(coordinator, _entry())
...
def test_next_delivery_none_without_moments():
    sensor = AmpReNextDeliverySensor(_coordinator([_parcel("A")]), _entry())
    assert sensor.native_value is None
    assert sensor.extra_state_attributes == {}
...
def test_delivered_sensor():
    ...
    sensor = AmpReDeliveredParcelsSensor(coordinator, _entry())
...
def test_last_update_sensor():
    ...
    sensor = AmpReLastUpdateSensor(coordinator, _entry())
